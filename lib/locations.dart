...
class Regions {
  ...
  Regions.fromJson(Map<String, dynamic> json) {
    coords =
    json['coords'] != null ? new Coordinate.fromJson(json['coords']) : null;
    id = json['id'];
    name = json['name'];
    zoom = json['zoom'];
  }
...
Future<Locations> getGoogleOffices() async {
  ...
  if (response.statusCode == 200) {
    print("fetched google locations successfully ${response.body}");
    return Locations.fromJson(json.decode(response.body));
  } else {
    throw HttpException('Unexpected status code ${response.statusCode}:'' ${response.reasonPhrase}',uri: Uri.parse(googleLocationsURL));
  }
}